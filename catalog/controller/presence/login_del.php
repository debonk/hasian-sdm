...
class ControllerPresenceLogin extends Controller {
	...
	public function verification() {
		...
		if (isset($this->request->get['action']) && $this->request->get['action'] == 'logout') {
			$schedule_date = date('Y-m-d', strtotime('-3 hours'));//penentuan tgl jadwal
			
			$action = 'logout';
		} else {
			$login_start = $this->config->get('payroll_setting_login_start') + 60;
			$schedule_date = date('Y-m-d', strtotime('+' . $login_start . ' minutes'));
	...
	public function validateLog() {
		...
		if (isset($this->request->post['action']) && $this->request->post['action'] == 'logout') {
			$schedule_date = date('Y-m-d', strtotime('-11 hours'));
...
//CEK UNTUK PERUBAHAN JADWAL AKIBAT EXCHANGE/ABSENCE/OVERTIME UTK WAKTU YG TLH LAMPAU
					...
					if (!$log_info) {
						if ($schedule_check) {
							...
							if ($schedule_info['time_in'] > $schedule_info['time_out']) {
						...
						$this->model_presence_presence->addLogSchedule($customer_id, $schedule_date, $time_in, $time_out);

						$log_info = $this->model_presence_presence->getLog($customer_id, $schedule_date);
					}
					
					if ($log_info['time_logout'] != '0000-00-00 00:00:00') {//Cek sudah logout
						$check = 1;
						$json['error'] = $this->language->get('error_logout');
					}
					
					if ($action == 'logout') {
						if ($log_info['time_login'] == '0000-00-00 00:00:00') {//Cek belum login
							$check = 1;
							...
							if ($logout_start && strtotime($datetime_logout) < strtotime($log_info['time_out'])) {//Cek login sebelum waktu start yang diizinkan
								$check = 1;
						...
						if ($log_info['time_login'] != '0000-00-00 00:00:00') {//Cek sudah login
							$check = 1;
							...
							if ($login_start && strtotime($date_login_start) < strtotime($log_info['time_in'])) {//Cek login sebelum waktu start yang diizinkan
								$check = 1;
								$json['error'] = $this->language->get('error_login_start');
								
							} elseif ($login_end && strtotime($date_login_end) > strtotime($log_info['time_in'])) {//Cek login setelah waktu akhir yang diizinkan
								$check = 1;
					...
					// if ($log_info) {
						...
						// } elseif ($action == 'login' && $log_info['time_login'] != '0000-00-00 00:00:00') {//Cek sudah login
							// $check = 1;
							// $json['error'] = $this->language->get('error_login');
