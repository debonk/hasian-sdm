			...
			<div class="panel-body">
				...
				<div class="table-responsive">
					<table class="table table-bordered table-hover">
						...
						<tbody>
							<?php if ($documents) { ?>
							<?php foreach ($documents as $document_type_id => $document_list) { ?>
							<?php foreach ($document_list['document_data'] as $document) { ?>
							<tr>
								<td class="text-left <?php echo $document_list['required']; ?>">
									<?php echo $document_list['title']; ?>
								</td>
								<td class="text-left">
								...
								<td class="text-right">
									...
									<button type="button" data-toggle="tooltip" title="<?php echo $button_delete; ?>"
										id="button-delete<?php echo $document['document_id']; ?>"
										value="<?php echo $document['document_id']; ?>" data-loading-text="<?php echo $text_loading; ?>"
										class="btn btn-danger"><i class="fa fa-trash-o"></i></button>
									<?php } else { ?>
									<button type="button" data-toggle="tooltip" title="<?php echo $button_upload; ?>"
										id="button-upload<?php echo $document_type_id; ?>" value="<?php echo $document_type_id; ?>"
	...
	<script type="text/javascript">
		...
		$('button[id^=\'button-delete\']').on('click', function (e) {
			if (confirm('<?php echo $text_confirm; ?>')) {
				...
				$.ajax({
					url: 'index.php?route=customer/document/delete&token=<?php echo $token; ?>&customer_id=<?php echo $customer_id; ?>&document_id=' + $(node).val(),
					dataType: 'json',
					crossDomain: false,
					beforeSend: function () {
						$(node).button('loading');
