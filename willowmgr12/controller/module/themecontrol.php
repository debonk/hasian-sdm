...
class ControllerModuleThemeControl extends Controller {
	...
	public function index() {   
		...
		$this->mdata['action'] = $this->url->link('module/'.$this->moduleName, 'token=' . $this->session->data['token'], 'SSL');
		$this->mdata['exportprofile_action'] = $this->url->link('module/'.$this->moduleName, 'exportprofile=1&token=' . $this->session->data['token'], 'SSL');

		$this->mdata['ajax_modules_position'] = $this->url->link('module/'.$this->moduleName."/ajaxsave", 'token=' . $this->session->data['token'], 'SSL');
		$this->mdata['ajax_massinstall'] = $this->url->link('module/'.$this->moduleName."", 'ajax_massinstall=1&token=' . $this->session->data['token'], 'SSL');
		$this->mdata['ajax_clearcache'] = $this->url->link('module/'.$this->moduleName."/clearcache", 'token=' . $this->session->data['token'], 'SSL');
		$this->mdata['ajax_compileless'] =HTTP_CATALOG.'?compileless=1';
		
		$this->mdata['live_customizing_url'] =  $this->url->link('module/'.$this->moduleName."/customize", 'store_id='.$this->store_id.'&token=' . $this->session->data['token'], 'SSL');
		...
		$this->response->setOutput($this->load->view(   'module/themecontrol/'.$this->moduleName.'.tpl', $this->mdata) );
	...
	public function customize(){ 
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			$this->session->data['success'] = $this->language->get('text_success');
			$this->response->redirect($this->url->link('module/'.$this->moduleName.'/customize', 'token=' . $this->session->data['token'], 'SSL'));
		}
		
		$this->mdata['heading_title'] = $this->language->get('heading_title');
		$this->mdata['back_url']     = $this->url->link('module/'.$this->moduleName, 'token=' . $this->session->data['token'], 'SSL');
		$this->mdata['upload_link']  = $this->url->link('module/'.$this->moduleName.'/upload', 'token=' . $this->session->data['token'], 'SSL');
		$this->mdata['button_save'] = $this->language->get('button_save');
		$this->mdata['button_cancel'] = $this->language->get('button_cancel');
		...
		$this->mdata['action'] = $this->url->link('module/'.$this->moduleName.'/customize', 'token=' . $this->session->data['token'], 'SSL');
		$this->mdata['cancel'] = $this->url->link('extension/module', 'token=' . $this->session->data['token'], 'SSL');
		$this->mdata['token'] = $this->session->data['token'];
		$this->mdata['themeName'] = $theme;
