...
class ModelReleaseFundAccount extends Model
{
	...
	public function getFundAccount($fund_account_id)
	{ //Used by: free_transfer
	...
	public function getFundAccounts($data = array())
	{
		if ($data) {
			$sql = "SELECT * FROM " . DB_PREFIX . "v_fund_account";

			if (isset($data['fund_account_ids'])) {
				$sql .= " WHERE fund_account_id IN (" . implode(',', array_map('intval', $data['fund_account_ids'])) . ")";
			}
