...
//End List
	public function addPayrollMethod($data) {
		foreach ($data['payroll_method'] as $language_id => $value) {
			if (isset($payroll_method_id)) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "payroll_method SET payroll_method_id = '" . (int)$payroll_method_id . "', language_id = '" . (int)$language_id . "', name = '" . $this->db->escape($value['name']) . "'");
			} else {
				$this->db->query("INSERT INTO " . DB_PREFIX . "payroll_method SET language_id = '" . (int)$language_id . "', name = '" . $this->db->escape($value['name']) . "'");
	...
	public function getPayrollMethodsOld($data = array()) {
		if ($data) {
			$sql = "SELECT * FROM " . DB_PREFIX . "payroll_method";
			...
			if (!$payroll_method_data) {
				$query = $this->db->query("SELECT payroll_method_id, name FROM " . DB_PREFIX . "payroll_method ORDER BY name");

				$payroll_method_data = $query->rows;

				$this->cache->set('payroll_method.', $payroll_method_data);
			}
